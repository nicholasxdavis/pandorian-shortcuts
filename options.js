...
(function() {
    ...
    function initializeElements() {
        ...
        elements.enableQRCode = document.getElementById('enableQRCode');
    }
    ...
    function attachEventListeners() {
        ...
        if (elements.enableQRCode) {
            elements.enableQRCode.addEventListener('change', handleKeyboardShortcutToggle);
        }
        ...
        if (showTourBtn) {
            showTourBtn.addEventListener('click', () => {
                // Clear onboarding completion flag and start tour
                chrome.storage.local.remove(['pandorian_onboarding_completed'], () => {
                    // Check if onboarding script is loaded and trigger it
                    if (typeof window.startOnboarding === 'function') {
                        window.startOnboarding();
                    } else {
                        // If not available, reload page
     ...
     */
    function loadData() {
        chrome.storage.sync.get(['shortcuts', 'enabled', 'keyboardShortcuts', 'keyboardShortcutSettings'], (result) => {
            ...
            // Load keyboard shortcut settings (default to enabled)
            const shortcutSettings = result.keyboardShortcutSettings || {
                enableQRCode: true
            };
            
            if (elements.enableQRCode) {
                elements.enableQRCode.checked = shortcutSettings.enableQRCode !== false;
            }
    ...
    /**
     * Handle keyboard shortcut toggle changes
     */
    function handleKeyboardShortcutToggle(e) {
        ...
        chrome.storage.sync.get(['keyboardShortcutSettings'], (result) => {
            ...
            if (settingKey) {
                ...
                chrome.storage.sync.set({ keyboardShortcutSettings: settings }, () => {
                    if (chrome.runtime.lastError) {
                        console.error('[Pandorian] Failed to save keyboard shortcut settings:', chrome.runtime.lastError);
                        showToast('Failed to save settings', 'error');
                        // Revert toggle
                        e.target.checked = !isEnabled;
                    } else {
                        showToast(isEnabled ? 'Shortcut enabled' : 'Shortcut disabled', 'success');
                    }
                });
            }
        });
    ...
    // Removed all bookmark sidebar functions
